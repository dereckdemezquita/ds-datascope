...
```{r warning=FALSE, message=FALSE}
data$manual <- lapply(list.files("./data/manual-download", pattern = "\\.csv$", full.names = TRUE, all.files = TRUE), vroom, delim = ",")

unique(unlist(lapply(data$manual, colnames)))

names(data$manual) <- list.files("./data/manual-download", pattern = "\\.csv$", all.files = TRUE) %>%
...
data$manual <- data$manual %>%
	...
	select(Year, Country, Code, region7, region23, eu15, region7eu, dplyr::everything())
```
...
data$countries %>%
	...
	mutate(region7 = factor(region7, levels = region7_factors),
			 region23 = factor(region23, levels = region23_factors),
			 region7eu = factor(region7eu, levels = region7eu_factors)) %>%
	full_join(select(data$manual, Code, Year, pop_total), by = c("Code", "Year")) %>%
	select(Year, Code, Country, region7, region23, eu15, region7eu, dplyr::everything())
```
